...
//********* Timer fired, creating, and sending the message ***********
	...
	event void Data_timer.fired() {
				...
				if (!busy && call AMSend.send(AM_BROADCAST_ADDR, &pkt, sizeof(NodesMsg)) == SUCCESS) {
					...
					//printf("Packet with the ID: %u to the destination address: %u form the source address: %u loaded with usage data: %u and temperature data: %u has been created and successfully sent. \n", pcktid, root, TOS_NODE_ID, usagerand, temprand);	//Uncomment please to see the debug statement
			}
	}
	...
	event void Retransmission_timer.fired() {
				...
				if (!busy && call AMSend.send(AM_BROADCAST_ADDR, &pkt, sizeof(NodesMsg)) == SUCCESS) {
					busy = TRUE;
					//printf("Packet with ID %u is sent and the retrasmission timer is started.\n", ackpktid);	//Uncomment to see the packet with the Packet ID is retransmitted
			}
	}
				...
				if (tonodein == TOS_NODE_ID) {
					...
					if (pktidin != pastpktid) {
						...
						if (pktidin == ackpktid) {
							...
							//printf("Packet with ID %u is not a duplicate and has been acked. Data timer is restarted and retransmission timer is off.\n", pktidin);//Uncomment to see the packet with the Packet ID is ACKed
						}
					}
				}
			}
