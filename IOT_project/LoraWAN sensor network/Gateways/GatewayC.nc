...
//********* Creating a packet and sent it ***********
	...
	task void sender(){
			...
			if (!busy && call AMSend.send(tonode, &pkt, sizeof(GatewayOutMsg)) == SUCCESS) {
				busy = TRUE;
				//printf("And the packet is successfully forwarded to the destination address: %u.\n", tonode);	//Uncomment please to see the debug statement
			}
	}
...
//********* Receiving and processing the packet ***********
	...
	event message_t* Receive.receive(message_t* msg, void* payload, uint8_t len) {
		...
		if (len == sizeof(GatewayMsg)) {
			...
			//printf("Packet with ID: %u from the source address: %u to the destination address: %u has been received. \n", pktid, nodeid, tonode);	//Uncomment please to see the debug statement
